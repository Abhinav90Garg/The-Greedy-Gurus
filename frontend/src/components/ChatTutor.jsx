...
import { Send, Paperclip, MessageSquare, Plus, Sparkles, Cpu } from 'lucide-react';
...
const ChatTutor = () => {
  ...
  const sendMessageToBackend = async (question) => {
    try {
      ...
      return "ERROR: Could not connect to the backend engine. Check if the server is running on port 8000.";
    }
  };
    ...
    <div className="h-screen bg-[#050505] text-white flex overflow-hidden font-sans">
      ...
      <aside className="w-72 border-r border-white/5 bg-black flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-2">
          <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest ml-2 mb-4">Chat History</p>
        ...
        <div className="p-4 border-t border-white/5 flex items-center gap-3">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="text-sm font-bold truncate text-white uppercase tracking-tighter">Gurleen Kaur</p>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold border ${msg.role === 'assistant' ? 'bg-purple-600/20 border-purple-500/30 text-purple-400' : 'bg-white/10 border-white/20'}`}>
                {msg.role === 'assistant' ? 'AI' : 'GK'}
              </div>
              <div className={`p-4 rounded-2xl max-w-2xl text-sm border ${msg.role === 'assistant' ? 'bg-white/5 border-white/5 text-gray-300' : 'bg-purple-600/10 border-purple-500/20 text-white'}`}>
                {msg.content}
              </div>
            </div>
          ))}
        ...
        <div className="p-8">
          <div className="max-w-4xl mx-auto relative group">
            ...
            <input 
              ...
              placeholder={loading ? "Analyzing query..." : "Ask the OS anything..."}
            />
            <button onClick={handleSend} className="absolute right-3 top-1/2 -translate-y-1/2 p-3 bg-white text-black rounded-full hover:bg-purple-500 hover:text-white transition-all shadow-xl">
              <Send size={18} />
            </button>
          </div>
        </div>
...
export default ChatTutor; // <-- THE MOST IMPORTANT LINE