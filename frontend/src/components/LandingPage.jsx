...
const LandingPage = () => {
  ...
  // Features list using the Icons namespace
  const features = [
      ...
      <main className="max-w-7xl mx-auto px-6 pt-52 pb-40 relative z-10 text-center">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <h1 className="text-6xl md:text-[8rem] font-black tracking-tighter leading-[0.85] mb-12">
            The Future of <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-white via-white to-white/20 block">Workflow.</span>
          </h1>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-60 pb-20 text-left">
          {features.map((feature, index) => (
            <motion.div
              ...
              className="group relative p-8 rounded-[2.5rem] border border-white/10 bg-white/5 hover:bg-white/[0.07] hover:border-purple-500/50 transition-all"
            >
      ...
      <footer className="border-t border-white/5 bg-black/40 backdrop-blur-md py-24 relative z-10">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-12 text-center md:text-left">
          <div className="space-y-8 md:space-y-0 md:flex md:gap-20">
            <div>
              <h4 className="text-2xl font-black mb-1 tracking-tighter uppercase italic">Gurleen Kaur</h4>
              <p className="text-gray-600 text-xs font-mono tracking-widest uppercase italic">2ND_YEAR // CU</p>
            </div>
            <div>
              <h4 className="text-2xl font-black mb-1 tracking-tighter uppercase italic">Abhinav Garg</h4>
              <p className="text-gray-600 text-xs font-mono tracking-widest uppercase italic">2ND_YEAR // CU</p>
            </div>
          </div>
          <div className="flex gap-4">
            <a href="#" className="p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-all border border-white/5 text-gray-400 hover:text-white"><Icons.Github size={22} /></a>
            <a href="#" className="p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-all border border-white/5 text-gray-400 hover:text-white"><Icons.Linkedin size={22} /></a>
            <a href="#" className="p-4 rounded-2xl bg-white/5 hover:bg-white/10 transition-all border border-white/5 text-gray-400 hover:text-white"><Icons.Mail size={22} /></a>
          </div>
        </div>
      </footer>
    </div>
